...
struct Params
{
    ...
    float min_u_threshold;
    float min_m_threshold;
    float max_u_threshold;
    float max_m_threshold;
};
...
PatSegmentor::PatSegmentor(Params in_params) {
    ...
    min_u_thresh = in_params.min_u_threshold;
    min_m_thresh = in_params.min_m_threshold;
    max_u_thresh = in_params.max_u_threshold;
    max_m_thresh = in_params.max_m_threshold;
}
...
void PatSegmentor::scan_for_bimodal_2(std::vector<std::string> data_paths){
    ...
    try {
        ...
        while (start_site < nr_sites - min_bimodal_window_len){ // 80
            ...
            while (((prop_m >= min_m_thresh and prop_u >= min_u_thresh and prop_m <= max_m_thresh and prop_u <= max_u_thresh) or num_fails <= 3) and end_site < nr_sites - 1){
                if (is_first){
                    num_fails = 0;
                    is_first = false;
                }
...
//                    tmp_bimodal_win_threshold = bimodal_win_threshold * 0.8;
                ...
                if (not (prop_m >= min_m_thresh and prop_u >= min_u_thresh and prop_m <= max_m_thresh and prop_u <= max_u_thresh)){
                    num_fails += 1;
                } else {
                    continue_count = 0;
                    length += 1;
...
Params parse_params(InputParser &input){
    ...
    pams.min_bimodal_window_len = 5;
    pams.bimodal_win_threshold = 0.20;
    pams.min_reads_per_window = 5;
    pams.min_u_threshold = 0.20;
    pams.min_m_threshold = 0.20;
    pams.max_u_threshold = 1.0;
    pams.max_m_threshold = 1.0;
...
//    }
    std::string min_u_threshold = input.getCmdOption("-min_u_threshold");
    if (!min_u_threshold.empty()) {
        pams.min_u_threshold = std::stof(min_u_threshold);
    }
    std::string min_m_threshold = input.getCmdOption("-min_m_threshold");
    if (!min_m_threshold.empty()) {
        pams.min_m_threshold = std::stof(min_m_threshold);
    }
    std::string max_u_threshold = input.getCmdOption("-max_u_threshold");
    if (!max_u_threshold.empty()) {
        pams.max_u_threshold = std::stof(max_u_threshold);
    }
    std::string max_m_threshold = input.getCmdOption("-max_m_threshold");
    if (!max_m_threshold.empty()) {
        pams.max_m_threshold = std::stof(max_m_threshold);
    }
...
int main( int argc, char *argv[])
{
    ...
    if (argc < 6){
        std::cerr << "Usage: segment PAT_PATH [PA15795760T_PATH...] -s START -n NR_SITES ";
